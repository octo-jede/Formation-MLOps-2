...
def predict_endpoint():
    ...
    for arg in request.args:
        if arg in received_data_df.columns:
            if isinstance('string', float):
                received_data_df[arg] = float(request.args.get(arg))
            else:
                received_data_df[arg] = request.args.get(arg)
        else:
            raise KeyError(f"Column '{arg}' does not exist")
    
    X=prepare_features(data=received_data_df, training_mode=False) 
    
    prediction=predict(X, model_path=MODEL_PATH)
